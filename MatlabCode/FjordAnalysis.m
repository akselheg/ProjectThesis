...
scatter(table2(:,1), table2(:,2))
scatter(table3(:,1), table3(:,2))
legend('Wave Hz < 0.55', '0.55 < Wave Hz < 0.65', 'Wave Hz > 0.65')
xlabel 'Relative wave direction [deg]',ylabel 'Vr [m/s]';
hold off
%%
table1 = [];table2 = [];table3 = [];
for i = 1: length(ForecastWaveSize_data)
...
y1 = polyval(p,x1);
plot(x1,y1)
legend('Wave Size < 0.12', '0.12 < Wave Size < 0.2', 'Wave Size > 0.2', 'Linear Regression')
hold off
%%
table1 = [];table2 = [];table3 = [];
for i = 1: length(ForecastWaveSize_data)
...
y1 = polyval(p,x1);
plot(x1,y1)
legend('Wave Hz < 0.55', '0.55 < Wave Hz < 0.65', 'Wave Hz > 0.65' , 'Linear Regression')
...
xlabel 'Measured Relative wind direction',ylabel 'Vr [m/s]';
hold off
%%
figure;
